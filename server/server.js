...
function contains(sock, list) {
  ...
  while (iIndex--) {
    if (
      list[iIndex].remoteAddress == sock.remoteAddress &&
      list[iIndex].remotePort == sock.remotePort
    ) {
      return iIndex;
    }
  }
...
net
  .createServer(function(sock) {
    ...
    EWTRACE("CONNECTED: " + sock.remoteAddress + ":" + sock.remotePort);
    ...
    sock.on("data", function(data) {
      ...
      if (_headContext != "8A" && _headContext != "80") {
        ...
        var find = _.find(socketList, function(item) {
          return (
            item.remoteAddress == sock.remoteAddress &&
            item.remotePort == sock.remotePort
          );
        });
...
app.start = function(httpOnly) {
  ...
  server.listen(_port, function() {
    ...
    var baseUrl =
      (httpOnly ? "http://" : "https://") + app.get("host") + ":" + _port;
