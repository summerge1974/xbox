...
module.exports = function(Common) {
   ...
   ExecuteSyncSQLResult = function (bsSQL, ResultObj, tx, Connect) {
    ...
    wx_CreateOrders = function(fee, openid) {
        return new Promise(function(resolve, reject) {
            var _fee = fee * 100;
            var url = process.env.global_wxurl+"/createorders?appId="+process.env.wxAppID+"&inside_no=0&fee="+_fee+"&notifyUrl=https://box.zlian-tech.com/api/weChatEvents/wxnotify&openid="+openid;
            console.log(url);
            needle.get(encodeURI(url), null, function(err, userInfo) {
